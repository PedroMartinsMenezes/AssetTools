...
{
    #region Double
    [TransferableStruct("Vector4d", "Vector4", 32)]
    [DebuggerDisplay("({X} {Y} {Z} {W})")]
    public struct FVector4d : ITransferable, ITagConverter
    {
        public double X;
        public double Y;
        public double Z;
    ...
    public class FVector4dListJsonConverter : JsonConverter<List<FVector4d>>
    {
        public override List<FVector4d> Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
        {
            ...
            return s.Length == 0 ? [] : s.Split(" | ").Select(x => x.Split(' ') is var v ? new FVector4d { X = double.Parse(v[0]), Y = double.Parse(v[1]), Z = double.Parse(v[2]), W = double.Parse(v[3]) } : default).ToList();
        }
    ...
    #region Float
    [TransferableStruct("Vector4f", "Vector4", 16)]
    [DebuggerDisplay("({X} {Y} {Z} {W})")]
    public struct FVector4f : ITransferable, ITagConverter
    {
        public float X;
        public float Y;
        public float Z;
    ...
    public class FVector4fArrayJsonConverter : JsonConverter<FVector4f[]>
    {
        ...
        {
            string s = reader.GetString();
            return s.Length == 0 ? [] : s.Split(" | ").Select(x => x.Split(' ') is var v ? new FVector4f { X = float.Parse(v[0]), Y = float.Parse(v[1]), Z = float.Parse(v[2]), W = float.Parse(v[3]) } : default).ToArray();
        }
    ...
    public class FVector4fListJsonConverter : JsonConverter<List<FVector4f>>
    {
        ...
        {
            string s = reader.GetString();
            return s.Length == 0 ? [] : s.Split(" | ").Select(x => x.Split(' ') is var v ? new FVector4f { X = float.Parse(v[0]), Y = float.Parse(v[1]), Z = float.Parse(v[2]), W = float.Parse(v[3]) } : default).ToList();
        }
    ...
    [TransferableStruct("Vector4", size1: 16, size2: 32)]
    [DebuggerDisplay("({X} {Y} {Z} {W})")]
    public struct FVector4 : ITransferable, ITagConverter
    {
        public double X;
        public double Y;
        public double Z;
