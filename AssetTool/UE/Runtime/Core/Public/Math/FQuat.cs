...
namespace AssetTool
{
    ...
    public class FQuat4dArrayJsonConverter : JsonConverter<FQuat4d[]>
    {
        public override FQuat4d[] Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
        {
            ...
            return s.Length == 0 ? [] : s.Split(" | ").Select(x => x.Split(' ') is var v ? new FQuat4d { X = double.Parse(v[0]), Y = double.Parse(v[1]), Z = double.Parse(v[2]), W = double.Parse(v[3]) } : default).ToArray();
        }
    ...
    public class FQuat4dListJsonConverter : JsonConverter<List<FQuat4d>>
    {
        ...
        {
            string s = reader.GetString();
            return s.Length == 0 ? [] : s.Split(" | ").Select(x => x.Split(' ') is var v ? new FQuat4d { X = double.Parse(v[0]), Y = double.Parse(v[1]), Z = double.Parse(v[2]), W = double.Parse(v[3]) } : default).ToList();
        }
    ...
    public class FQuat4fArrayJsonConverter : JsonConverter<FQuat4f[]>
    {
        ...
        {
            string s = reader.GetString();
            return s.Length == 0 ? [] : s.Split(" | ").Select(x => x.Split(' ') is var v ? new FQuat4f { X = float.Parse(v[0]), Y = float.Parse(v[1]), Z = float.Parse(v[2]), W = float.Parse(v[3]) } : default).ToArray();
        }
    ...
    public class FQuat4fListJsonConverter : JsonConverter<List<FQuat4f>>
    {
        ...
        {
            string s = reader.GetString();
            return s.Length == 0 ? [] : s.Split(" | ").Select(x => x.Split(' ') is var v ? new FQuat4f { X = float.Parse(v[0]), Y = float.Parse(v[1]), Z = float.Parse(v[2]), W = float.Parse(v[3]) } : default).ToList();
        }
