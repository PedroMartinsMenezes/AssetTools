...
namespace AssetTool
{
    ...
    public class FInt64PropertyJsonArray : Dictionary<string, object>, IPropertytag
    {
        public const string Pattern = "long\\[\\] '(.*)'\\s*(?:\\[(\\d+)\\])?\\s*(?:\\(([-a-fA-F0-9]+)\\))?";

        public FInt64PropertyJsonArray() { }

        public FInt64PropertyJsonArray(FPropertyTag tag)
        {
            CultureInfo.DefaultThreadCurrentCulture = CultureInfo.InvariantCulture;
            string arrayIndex = tag.ArrayIndex > 0 ? $"[{tag.ArrayIndex}]" : string.Empty;
            string guidValue = tag.HasPropertyGuid == 0 ? string.Empty : $" ({tag.GuidValue})";
            string values = string.Join(' ', (tag.Value as List<object>).Select(x => x.ToString()));
            Add($"long[] '{tag.Name.ToString()}'{arrayIndex}{guidValue}", values);
        }
        ...
        public static FPropertyTag GetNative(Transfer transfer, string key, string value)
        {
            ...
            List<object> values = value.Length == 0 ? [] : value.Split(' ').Select(x => (object)Int64.Parse(x)).ToList();
            ...
            return new FPropertyTag
            {
                ...
                InnerType = new FName(FInt64Property.TYPE_NAME, transfer),
                Value = values,
                Size = size,
