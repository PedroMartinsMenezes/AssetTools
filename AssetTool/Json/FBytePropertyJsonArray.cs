...
namespace AssetTool
{
    ...
    public class FBytePropertyJsonArray : Dictionary<string, object>, IPropertytag
    {
        public const string Pattern = "byte\\[\\] '(.*)'\\s*(?:\\[(\\d+)\\])?\\s*(?:\\(([-a-fA-F0-9]+)\\))?";

        public FBytePropertyJsonArray() { }

        public FBytePropertyJsonArray(FPropertyTag tag)
        {
