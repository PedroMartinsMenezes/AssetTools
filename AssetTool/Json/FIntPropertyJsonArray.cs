...
{
    [DebuggerDisplay("int[]")]
    public class FIntPropertyJsonArray : Dictionary<string, object>, IPropertytag
    {
        public const string Pattern = "int\\[\\] '(.*)'\\s*(?:\\[(\\d+)\\])?\\s*(?:\\(([-a-fA-F0-9]+)\\))?";

        public FIntPropertyJsonArray() { }

        public FIntPropertyJsonArray(FPropertyTag tag)
        {
            string arrayIndex = tag.ArrayIndex > 0 ? $"[{tag.ArrayIndex}]" : string.Empty;
            string guidValue = tag.HasPropertyGuid == 0 ? string.Empty : $" ({tag.GuidValue})";
            string values = string.Join(' ', (tag.Value as List<object>).Select(x => x.ToString()));
            Add($"int[] '{tag.Name.ToString()}'{arrayIndex}{guidValue}", values);
        }
        ...
        public static FPropertyTag GetNative(Transfer transfer, string key, string value)
        {
            ...
            List<object> values = value.Length == 0 ? [] : value.Split(' ').Select(x => (object)Int32.Parse(x)).ToList();
            ...
            return new FPropertyTag
            {
                ...
                InnerType = new FName(FIntProperty.TYPE_NAME, transfer),
                Value = values,
                Size = size,
